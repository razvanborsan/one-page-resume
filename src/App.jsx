...
import { applyResumeStyles, COLORS } from "./lib/resumeStyles";
import {
  PAGE_WIDTH, PAGE_HEIGHT, DEFAULT_PADDING, BODY_FONT, MONO_FONT,
  LINE_HEIGHT_MIN, LINE_HEIGHT_MAX, LINE_HEIGHT_DEFAULT, DEFAULT_MAX_FONT_SIZE,
  measureBlocks, layoutBlocks, findOptimalFit,
} from "./lib/layout";
...
export default function App() {
  ...
  const [lineHeightMultiplier, setLineHeightMultiplier] = useState(LINE_HEIGHT_DEFAULT);
  ...
  const spacing = useMemo(
    () => ({ section: sectionSpacing, item: itemSpacing, separator: separatorSpacing }),
  ...
  const { measuredHeight, measureTime } = useMemo(() => {
    ...
    const h = measureBlocks(blocks, { fontSize, contentWidth, lineHeightMultiplier, spacing });
    return {
      measuredHeight: Math.round(h),
      measureTime: +(performance.now() - t0).toFixed(2),
    };
  }, [blocks, fontSize, contentWidth, lineHeightMultiplier, spacing, fontsLoaded]);

  const positionedItems = useMemo(() => {
    if (!fontsLoaded) return [];
    return layoutBlocks(blocks, { fontSize, contentWidth, padding, lineHeightMultiplier, spacing });
  }, [blocks, fontSize, contentWidth, padding, lineHeightMultiplier, spacing, fontsLoaded]);

  useEffect(() => {
    if (!fontsLoaded || !autoFit) return;
    const { fontSize: optFs, lineHeightMultiplier: optLh } = findOptimalFit(blocks, {
      contentWidth, maxContentHeight, maxFontSize, spacing,
    });
    setFontSize(Math.min(optFs, maxFontSize));
    setLineHeightMultiplier(optLh);
  }, [blocks, autoFit, fontsLoaded, contentWidth, maxContentHeight, maxFontSize, spacing]);
      ...
      <main className="flex-1 flex flex-col sm:flex-row bg-neutral-900 text-white min-h-0 overflow-hidden">
                    ...
                    ? item.parts.map((part, j) => (
                        <span
                          ...
                          style={{
                            ...
                            backgroundColor: part.code ? COLORS.inlineCodeBg : undefined,
        ...
        <div className={`w-full sm:w-72 sm:shrink-0 self-stretch sm:border-l border-neutral-800 px-5 py-4 flex-col gap-4 overflow-y-auto ${activeTab === "settings" ? "flex" : "hidden"} sm:flex`}>
          ...
          <SliderControl
            ...
            min={LINE_HEIGHT_MIN} max={LINE_HEIGHT_MAX} step={0.001}
          ...
          <div>
            ...
            <div className="flex items-baseline justify-between text-xs">
              <span className={`font-medium ${fits ? "text-emerald-400" : "text-red-400"}`}>
                {fits ? "Fits on 1 page" : `Overflow +${measuredHeight - maxContentHeight}px`}
              </span>
